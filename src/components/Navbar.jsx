      ...
      <div className="flex w-full xl:flex-col items-center justify-between xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] xl:h-max py-8 bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full">
        {navLinks.map((link, index) => (
          <Link
            className={
              link.path === pathname
                ? "bg-gray-100 p-3 rounded-full text-black-100"
                : "relative flex items-center group hover:bg-slate-400 hover:rounded-full hover:p-3 transition-all duration-300"
            }
            to={link.path}
            key={index}
          >
            <div className="absolute pl-16 left-0 hidden xl:group-hover:flex">
              <div className="bg-white relative flex text-primary items-center p-[6px] rounded-sm">
                <div className="text-[16px] leading-none font-medium capitalize ">
                  {link.name}
                </div>
